  ...
  <div class="container">
    ...
    <div v-else>
      ...
      <div v-if="changes.length > 0">
        <button type="button" @click="checkTaskChangesFromServer(changes)">
          Сохранить
        </button>
        <button type="button" @click="cancelEdit()">Отменить</button>
      </div>
      <div ref="gantt" :class="`gantt--${stateScale}`" class="gantt"></div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    refreshTasksData(tasks) {
      tasks.forEach((newTask) => {
        ...
        if (newTask.type === gantt.config.types.task) {
          if (oldTask.start_date)
            oldTask.start_date = dateToString(newTask.start_date);
          if (oldTask.end_date)
            oldTask.end_date = dateToString(newTask.end_date);
        }
    ...
    setTasksConfig() {
      ...
      templates.leftside_text = (start, end, task) => {
        if (task.time && task.time_used)
          return `(${minToHour(task.time)} - ${minToHour(task.time_used)})`;
      };
      gantt.templates.scale_cell_class = (date) =>
        setWorkDays(date, this.stateScale);
      templates.timeline_cell_class = (task, date) =>
        `${setWorkDays(date, this.stateScale)} ${dateToLocaleString(date)}`;
    ...
    setDayWorkLoadCell() {
      gantt.templates.task_text = (start, end, task) => {
        const isReadonly =
          this.tasks.readonly || task.type !== gantt.config.types.task;
        if (this.stateScale === "day") {
          return task.loads.day
            .map((l) => {
              let className = "";
